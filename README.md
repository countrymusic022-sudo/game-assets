...
- 自動分段（段落 + 時間軸）
- 匯出並下載 `SRT` 字幕
- 下載 `TXT`、檢視 `JSON`（方便二次處理）
- 三種辨識模式：Fast / Balanced / Accurate
- 可選：匯入台灣流行用語字典做後處理修正
...
- **Q: 如果出現 hallucination（例如同一句重複很多次）？**  
  A: 系統已啟用較嚴格過濾：`no_speech_prob / avg_logprob / compression_ratio` 門檻、VAD 強化、關閉 `condition_on_previous_text`、以及重複詞句去重與「單段最多 8 秒」強制切段。
